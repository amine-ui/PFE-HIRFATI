...
class ArtisanController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        // }
        $old_user_image=$artisan->image;
        if (!$request->file('image')) {
            $profile_image =$old_user_image;
            return;
        }else{
            $profile_image=$request->file('image')->store("profile", "public");
        }
        $artisan->image=$profile_image;
        $artisan->update();
    ...
    public function destroy(Request $request)
    {
        ...
        if ($artisan) {
            ...
            if($artisan->image !="profile/user.png"){
                unlink("storage/".$artisan->image);
            }
            $artisan->delete();
